...
(() => {
  ...
  const stylesheet = `
     ...
     * Only the stage/rail hide — the stage background stays, so the
  ...
  class DeckStage extends HTMLElement {
    ...
    _restoreIndex() {
      // Deep-linking via location.hash has been disabled so the deck
      // always starts at the authored initial slide (index 0).
      // Hosts that need deep-linking can implement it externally.
    }
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      // history.replaceState calls for per-slide hashes removed to keep
      // a single canonical URL for the deck (no #1, #2 links).
    ...
    _onMessage(e) {
      ...
      if (d && d.__dc_op_ack) {
        ...
        if (d.applied === false && this._indexBeforeEmit != null) {
          ...
          // suppressed deep-link hash restore
        }
    ...
    _emitDcOp(op, slide, lock, newIndex) {
      ...
      if (op.emitOnly) {
        ...
        if (newIndex != null && newIndex !== this._index) {
          ...
          // suppressed optimistic deep-link hash update
        }
      }
